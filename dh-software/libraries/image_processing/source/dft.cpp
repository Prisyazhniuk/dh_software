...
namespace dh
{
    dft::dft( int width, int height, int channels )
        : _width( width )
        , _height( height )
        , _channels( channels )
    {
        ...
        auto status = ippiDFTGetSize_R_32f( { _width, _height }, _flags, _hint,
                                            &context_size, &init_buffer_size, &work_buffer_size );

        if( status != ippStsNoErr )
        {
            auto msg = dh_string::fs( "ippiDFTGetSize_R_32f failed: %s", ippGetStatusString( status ) );
            throw image_processing_exception( msg, get_exception_source() );
        }

        _context = reinterpret_cast<IppiDFTSpec_R_32f*>( ippsMalloc_8u( context_size ) );
        if( !_context )
        {
        ...
        status = ippiDFTInit_R_32f( { _width, _height }, _flags, _hint, _context, _init_buffer );
        if( status != ippStsNoErr )
        {
            auto msg = dh_string::fs( "ippiDFTInit_R_32f failed: %s", ippGetStatusString( status ) );
            throw image_processing_exception( msg, get_exception_source() );
        }
    ...
    void dft::forward( const Mat& src, Mat& magnitudes )
    {
        if( src.cols != _width )
        ...
        auto status = ippiDFTFwd_RToPack_32f_C1R( src.ptr<float>(), static_cast<int>( src.step ),
